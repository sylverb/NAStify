//
//  ServerSettingsOwnCloudViewController.m
//  NAStify
//
...
@implementation ServerSettingsOwnCloudViewController
            ...
            switch (indexPath.row)
            {
                ...
                case 2:
                {
                    ...
                    [textCellPath setCellDataWithLabelString:NSLocalizedString(@"Path:",@"")
                                             ...
                                             withPlaceHolder:NSLocalizedString(@"/owncloud",@"")
                                                    isSecure:NO
                                            withKeyboardType:UIKeyboardTypeDefault
                                                withDelegate:self
                                                      andTag:PATH_TAG];
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    switch (textField.tag)
    {
        ...
        case ADDRESS_TAG:
        {
            ...
            {
                NSString *path = [address substringFromIndex:[[addressArray objectAtIndex:0] length]];
                if ([path hasSuffix:@"/"])
                {
                    path = [path substringToIndex:[path length]-1];
                }
                address = [addressArray objectAtIndex:0];
                [self.localSettings setObject:path forKey:@"path"];
            }
            ...
            {
                self.userAccount.port = [addressArray objectAtIndex:1];
                address = [addressArray objectAtIndex:0];
            }
            
            self.userAccount.server = address;
            [self.tableView reloadData];
            break;
        }
        case PORT_TAG:
