...
info "Building SSH2 for '${OSSTYLE}'"
info "Using ${ARCH} with SDK version ${SDK_VERSION}"

THIS_SCRIPT_PATH=`pwd`/$0

spushd `dirname ${THIS_SCRIPT_PATH}`/libssh2-${LIBSSH2_VERSION}
SSH2ROOT=`pwd` # Let's make sure SSH2ROOT is an absolute path
spopd
...
BUILDDIR="${SSH2ROOT}/build-ios-${OSSTYLE}${PLATFORM}/${ACTUAL_ARCH}"
PREFIX="${SSH2ROOT}/install-ios-${OSSTYLE}${PLATFORM}/${ACTUAL_ARCH}"
...
${SSH2ROOT}/configure \
    ...
    --with-openssl \
    --disable-shared \
--with-libssl-prefix=${SSH2ROOT}/../../openssl/bin/openssl-${OPENSSL_VERSION}-${OSSTYLE}-${ACTUAL_ARCH} > ${out}
