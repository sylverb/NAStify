...
spushd `dirname ${THIS_SCRIPT_PATH}`/libidn-${LIBIDN_VERSION}
IDNROOT=`pwd` # Let's make sure IDNROOT is an absolute path
spopd
...
BUILDDIR="${IDNROOT}/build-ios-${PLATFORM}/${ARCH}"

PREFIX="${IDNROOT}/install-ios-${PLATFORM}/${ARCH}"
...
#export CFLAGS="${CFLAGS} -Wno-trigraphs -fpascal-strings -fasm-blocks -Wreturn-type -Wunused-variable -fmessage-length=0 -fvisibility=hidden"
...
${IDNROOT}/configure \
    ...
    --host="${TARGET}" \
    --enable-static \
    --disable-shared > ${out} # MMX and SSE support requires llvm which is broken on Simulator
